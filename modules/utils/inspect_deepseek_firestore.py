...
def find_latest_deepseek_import():
    ...
    query = chats_ref.where('title', '==', 'Import (DeepSeek)').order_by('createdAt', direction=firestore.Query.DESCENDING).limit(1)
    ...
    if not results:
        ...
        all_chats = chats_ref.order_by('createdAt', direction=firestore.Query.DESCENDING).limit(20).stream()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Inspiziert DeepSeek-Import in Firestore")
    parser.add_argument('--chat-id', help="Spezifische Chat-ID (oder leer für neuesten Import)")
