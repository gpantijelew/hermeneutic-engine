...
def get_chat_list_standalone() -> List[Dict]:
    ...
    try:
        chats = []
        docs = db.collection('chats').order_by(
            'lastUpdated', 
...
def delete_chat_standalone(chat_id: str) -> bool:
    ...
    try:
        ...
        print(f"   ✅ Chat-Dokument gelöscht")
...
def find_deepseek_imports():
    ...
    for chat in all_chats:
        ...
        is_deepseek = (
            'deepseek' in title.lower() or
            'import (deepseek)' in title.lower() or
            'vergleich chinesischer ki' in title.lower() or
            'ki-modelle: vergleich' in title.lower()  # Unser fehlerhafter Import
        )
...
def interactive_mode():
    ...
    for i, imp in enumerate(deepseek_imports, 1):
        status = "❌ LEER" if imp['message_count'] == 0 else f"✅ {imp['message_count']} Messages"
...
def main():
    ...
    parser.add_argument(
        '--chat-id',
        help="Direkt eine Chat-ID löschen (überspringt interaktiven Modus)"
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help="Simulation (keine echten Änderungen)"
    )
    parser.add_argument(
        '--list',
        action='store_true',
        help="Nur DeepSeek-Imports auflisten"
    )
    ...
    if args.list:
        ...
        if imports:
            ...
            for imp in imports:
                print(f"  - {imp['title']} (ID: {imp['id']}, Messages: {imp['message_count']})")
        else:
