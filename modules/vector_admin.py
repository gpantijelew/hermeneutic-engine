...
def render_chat_label_editor():
    ...
    # Label Wahl
    available_labels = [
        "Kimi", "DeepSeek", "ChatGPT", "Claude", 
        "Gemini", "Grok", "Perplexity", "GLM-4.6", "Unbekannt"
    ]
    ...
    if col2.button("🔄 Indexieren", key="btn_reindex"):
        with st.sidebar.status("Arbeite...") as status:
            try:
                ...
                st.sidebar.success(
                    f"✅ {count} Chunks als '{new_label}' indiziert."
                )
                time.sleep(2)
                st.rerun()
...
def render_label_overview():
    ...
    with st.sidebar.expander("📊 Label-Übersicht", expanded=False):
        ...
        labeled = [
            c for c in chats 
            if c['model_name'] not in ['Unbekannt', None, '']
        ]
        unlabeled = [
            c for c in chats 
            if c['model_name'] in ['Unbekannt', None, '']
        ]
            ...
            if col2.button("🔄", key=f"reindex_{chat_id}", help="Re-indizieren"):
                with st.spinner(f"Re-indiziere {title}..."):
                    try:
                        ...
                        st.success(
                            f"✅ {total} Chunks indiziert, "
                            f"{skipped_new} übersprungen."
                        )
                        st.rerun()
                    except Exception as e:
                        st.error(f"Fehler: {e}")