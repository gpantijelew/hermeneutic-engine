...
class BaseImporter(ABC):
    ...
    - import_to_db(): Schreibt Nachrichten in DB
    """
    ...
    def import_to_db(
        ...
        """
        Schreibt Nachrichten in die Datenbank.
...
class HTMLImporter(BaseImporter):
    ...
    def parse_html(self, html_content: bytes) -> BeautifulSoup:
        ...
        return BeautifulSoup(
            html_content.decode("utf-8", errors="ignore"), "html.parser"
        )
    ...
    def extract_thinking_block(self, element) -> Optional[str]:
        ...
        if thought_container:
            ...
            raw_thought = re.sub(
                r"^\s*Thought for \d+ seconds?\s*", "", raw_thought, flags=re.IGNORECASE
            ).strip()
...
class ConfigBasedImporter(HTMLImporter):
    ...
    def _html_to_markdown(self, element) -> str:
        ...
        # 2. Fettdruck
        html_str = re.sub(
            r"<strong[^>]*>(.*?)</strong>", r"**\1**", html_str, flags=re.DOTALL
        )
        ...
        html_str = re.sub(
            r"<ul[^>]*>(.*?)</ul>", replace_list, html_str, flags=re.DOTALL
        )
        html_str = re.sub(
            r"<ol[^>]*>(.*?)</ol>", replace_list, html_str, flags=re.DOTALL
        )

        # 6. Code-Blöcke
        html_str = re.sub(
            r"<code[^>]*>(.*?)</code>", r"`\1`", html_str, flags=re.DOTALL
        )
        html_str = re.sub(
            r"<pre[^>]*>(.*?)</pre>", r"\n```\n\1\n```\n", html_str, flags=re.DOTALL
        )
        ...
        html_str = re.sub(
            r"<table[^>]*>.*?</table>", replace_table, html_str, flags=re.DOTALL
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if not message_blocks:
                warning_msg = "⚠️ Config-Parser: Keine Nachrichtenblöcke gefunden."
                if container:
    ...
    def import_to_db(
        ...
        chat_id = create_chat(f"Import{title_suffix}")
