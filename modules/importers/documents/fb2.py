...
class FB2Importer(BaseImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if container:
                container.warning(
                    "⚠️ 'lxml' fehlt. FB2-Parsing könnte ungenau sein. `pip install lxml` empfohlen."
                )
        ...
        try:
                ...
                if desc:
                    ...
                    if t_info:
                        ...
                        if a_tag:
                            ...
                            parts = []
                            if f_name:
                                parts.append(f_name.get_text(strip=True))
                            if l_name:
                                parts.append(l_name.get_text(strip=True))
                            if parts:
                                author = " ".join(parts)
            except Exception as e:
                if container:
                    container.warning(f"⚠️ Metadaten-Fehler: {e}")
            ...
            for body in bodies:
                ...
                text = re.sub(
                    r"\b(l|d|n|j|m|t|s|c|qu)'\s+", r"\1'", text, flags=re.IGNORECASE
                )
                text = re.sub(
                    r"\b(l|d|n|j|m|t|s|c|qu)’\s+", r"\1’", text, flags=re.IGNORECASE
                )
            ...
            if not full_text_parts:
                if container:
                    container.warning("⚠️ FB2 scheint leer zu sein.")
                return []
            ...
            if container:
                container.success(
                    f"✅ Import fertig: {len(messages)} Abschnitte generiert."
                )
            ...
            if container:
                container.error(f"❌ FB2 Fehler: {e}")
            return []
    ...
    def import_to_db(
        ...
        chat_title = (
            f"📖 {title_match.group(1).strip()}" if title_match else "FB2 Import"
        )
