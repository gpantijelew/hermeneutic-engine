...
class EPubImporter(BaseImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not full_text_parts:
                if container:
                    container.warning(
                        "⚠️ ePub scheint leer zu sein (oder DRM-geschützt)."
                    )
                return []
            ...
            if container:
                container.error(f"❌ ePub Fehler: {e}")
            return []
    ...
    def import_to_db(
        ...
        chat_title = (
            f"📖 {title_match.group(1).strip()}" if title_match else "eBook Import"
        )
