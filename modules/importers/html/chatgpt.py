...
class ChatGPTImporter(ConfigBasedImporter):
    ...
    def parse(
        self, content: Any, progress_callback: Optional[Any] = None, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not role_elements:
            ...
            if container:
                container.info(
                    "⚠️ Standard-Selektor fehlgeschlagen, versuche Fallback..."
                )
        ...
        for i, el in enumerate(role_elements):
            ...
            # Content Extraktion
            content_div = el.select_one(".markdown") or el.select_one(
                ".whitespace-pre-wrap"
            )
            ...
            if full_text.strip():
                msg_obj = {"role": final_role, "content": full_text}
                ...
                model_badge = el.find_previous(
                    lambda tag: (
                        tag.name == "div" and re.search(r"GPT-|o1-|o3-", tag.get_text())
                    )
                )
                if model_badge:
