...
class DeepSeekImporter(ConfigBasedImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        if user_wrappers:
            if container:
                container.info(
                    f"DeepSeek: Präzise Extraktion ({len(user_wrappers)} Message-Paare)"
                )
        ...
        if all_msgs:
            if container:
                container.info(
                    f"DeepSeek: Standard-Format erkannt ({len(all_msgs)} Nachrichten)"
                )
        ...
        if root:
            if container:
                container.warning(
                    "DeepSeek: Stark obfuscated Format. Verwende Heuristik..."
                )
        ...
        if container:
            container.warning(
                "DeepSeek: Keine Struktur gefunden. Versuche Text-Block-Extraktion..."
            )
