        ...
        try:
            ...
            for block in chat_blocks:
                ...
                # MODEL RESPONSE (Arena Turn)
                    ...
                    model_cards = block.select(
                        "div.bg-surface-primary.relative.flex.w-full"
                    )
                    ...
                    for card in model_cards:
                        ...
                        thought_text = ""
                        reasoning_element = card.select_one(
                            'div[data-sentry-component="ReasoningContent"]'
                        )
                        if reasoning_element:
                            raw_thought = reasoning_element.get_text(
                                separator="\n", strip=True
                            )
                            raw_thought = re.sub(
                                r"^Thought for \d+ seconds", "", raw_thought
                            ).strip()
                        ...
                        if content_element:
                            main_text = content_element.get_text(
                                separator="\n", strip=True
                            )
            ...
            if container:
                container.success(
                    f"LM Arena Parser: {len(messages)} Interaktionen extrahiert."
                )
    ...
    def import_to_db(
        ...
        for msg in messages:
            if msg["role"] == "user":
                ...
                models = msg.get("models", [])
                if len(models) >= 1:
                    chat_a_history.append(
                        {"role": "model", "content": models[0]["content"]}
                    )
                if len(models) >= 2:
                    chat_b_history.append(
                        {"role": "model", "content": models[1]["content"]}
                    )

        # Titel generieren
        if container:
            container.info("🧠 Generiere Titel-Zusammenfassung...")
        topic_a = get_topic_summary(chat_a_history)

        # Chat A speichern
        title_a = f"Arena: {model_a_name} | {topic_a}"
        chat_id_a = create_chat(title_a)
        count_a = 0
        if chat_id_a:
            for msg in chat_a_history:
                ...
                if save_message(
                    chat_id_a, msg["role"], msg["content"], metadata=msg_meta
                ):
                    count_a += 1

        # Chat B speichern
        title_b = f"Arena: {model_b_name} | {topic_a}"
        chat_id_b = create_chat(title_b)
        count_b = 0
        if chat_id_b:
            for msg in chat_b_history:
                ...
                if save_message(
                    chat_id_b, msg["role"], msg["content"], metadata=msg_meta
                ):
                    count_b += 1
