...
class KimiImporter(ConfigBasedImporter):
    ...
    @property
    def platform_name(self): return "Kimi Chat"

    @property
    def platform_id(self): return "kimi"
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        if not items:
            if container: container.warning("⚠️ Keine Kimi-Nachrichtenblöcke gefunden.")
            return []
        ...
        for item in items:
            ...
            if not content_div:
                content_div = item.find(class_=lambda x: x and ("text" in x or "content" in x))
