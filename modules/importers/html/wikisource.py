...
class WikisourceImporter(HTMLImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            page_title = (
                ...
                if title_element
                else "Wikisource Dokument"
            )

            # Autor (falls vorhanden in Metadaten)
            author_meta = soup.find("meta", attrs={"name": "author"})
            author = (
                author_meta.get("content", "Unbekannt") if author_meta else "Unbekannt"
            )
            ...
            cleaned_text = re.sub(
                r"[a-z]+:[a-z]+\}\.ve-ce-[a-zA-Z\-\.]+\{[^\}]*\}", "", cleaned_text
            )
            ...
            if container:
                container.success(
                    f"✅ Wikisource-Import: {len(cleaned_text)} Zeichen extrahiert."
                )
    ...
    def import_to_db(
        ...
        """
        Speichert Wikisource-Content in der Datenbank.
