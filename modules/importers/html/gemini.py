...
class GeminiImporter(ConfigBasedImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        angular_elements = soup.find_all(
        ...
        if angular_elements:
            if container:
                container.info(
                    f"🔄 Strategie 1 (Angular): {len(angular_elements)} Elemente gefunden."
                )
            ...
            for i, elem in enumerate(angular_elements):
                ...
                # 2. THINKING (sammeln für nächste Model-Message)
                    ...
                    if thought_div:
                        ...
                        if debug_mode and container:
                            container.write(
                                f"💭 Thinking-Block gesammelt: {raw_thought[:50]}..."
                            )
            ...
            if messages:
                ...
                if container:
                    container.info(
                        f"📊 Strategie 1: {len(messages)} Nachrichten ({user_count} User / {model_count} Model)"
                    )
                ...
                if user_count > 0:
                    ...
                    if self.validate(messages):
                        if container:
                            container.success("✅ Strategie 1 erfolgreich!")
                        return messages
                    else:
                        if container:
                            container.warning(
                                "⚠️ Strategie 1: Validierung fehlgeschlagen. Versuche Strategie 2..."
                            )
                else:
                    if container:
                        container.warning(
                            "⚠️ Strategie 1: Keine User-Nachrichten. Versuche Strategie 2..."
                        )
        ...
        if all_boxes:
            if container:
                container.info(
                    f"🔄 Strategie 2 (Takeout): {len(all_boxes)} Message-Boxen gefunden."
                )
            ...
            if messages and self.validate(messages):
                if container:
                    container.success(
                        f"✅ Strategie 2 erfolgreich: {len(messages)} Nachrichten."
                    )
                return messages
            elif container:
        ...
        if target_word:
            if container:
                container.warning(
                    f"🔎 Strategie 3 (Diagnose): Deep Scan nach '{target_word}'..."
                )

            found_elements = soup.find_all(
                string=re.compile(re.escape(target_word), re.IGNORECASE)
            )

            if found_elements:
                if container:
                    container.success(
                        f"✅ {len(found_elements)} Treffer gefunden! Zeige Kontexte..."
                    )
                    ...
                    for element in found_elements:
                        ...
                        container.markdown(
                            f"**Treffer {count + 1} (Tag: {parent.name}):**"
                        )
                ...
                if container:
                    container.error(
                        f"❌ Das Wort '{target_word}' wurde im HTML nicht gefunden."
                    )
                return []
        ...
        if container:
            container.error(
                "❌ Alle Strategien fehlgeschlagen. HTML-Struktur unbekannt."
            )
