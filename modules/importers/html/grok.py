...
class GrokImporter(HTMLImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        if container and messages:
            ...
            container.success(
                f"✅ {len(messages)} Nachrichten: {user_count} User, {ai_count} AI"
            )
    ...
    def _html_to_markdown(self, element) -> str:
        ...
        def replace_code_block(match):
            ...
            if "class=" in match.group(0):
                lang_match = re.search(
                    r'class=["\'](?:language-)?(\w+)', match.group(0)
                )
                if lang_match:
        ...
        html_str = re.sub(
            r"<pre[^>]*>(.*?)</pre>", replace_code_block, html_str, flags=re.DOTALL
        )
    ...
    def import_to_db(
        ...
        # Chat erstellen
        chat_id = create_chat("Import (Grok)")
