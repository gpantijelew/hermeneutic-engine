...
class PerplexityImporter(HTMLImporter):
    ...
    def parse(self, content: Any, **kwargs) -> List[Dict[str, Any]]:
        ...
        if not user_blocks:
            if container:
                container.warning(
                    "⚠️ Perplexity: Keine User-Nachrichten gefunden (span.select-text)"
                )
            return []

        # 2. Finde alle AI-Responses
        # WICHTIG: Beide Klassen müssen vorhanden sein!
        ai_blocks = soup.find_all(
            "div", class_=lambda x: x and "prose" in x and "dark:prose-invert" in x
        )

        if not ai_blocks:
            if container:
                container.warning(
                    "⚠️ Perplexity: Keine AI-Antworten gefunden (div.prose)"
                )
            return []

        if container:
            container.info(
                f"Perplexity: {len(user_blocks)} User-Nachrichten, {len(ai_blocks)} AI-Antworten"
            )
    ...
    def import_to_db(
        ...
        # Chat erstellen
        chat_id = create_chat("Import (Perplexity AI)")
