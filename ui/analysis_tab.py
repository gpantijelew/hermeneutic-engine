...
def _render_results_block(chat_map: dict) -> None:
        ...
        if st.button("🕵️‍♂️ Tiefenprüfung starten (Faktencheck)"):
            ...
            with st.spinner("Der Enforcer prüft parallel (v49.1 Speedup)..."):
                ...
                if checked_count == 0:
                    ...
                    st.balloons()
                    st.success(
                        f"🎉 Perfekt! {checked_count} Fakten erfolgreich verifiziert."
                    )
    ...
    for i, res in enumerate(results):
        ...
        header_text = (
            f"[{i + 1}] {icon} {platform} | {score:.1f}% Relevanz | {chat_title}"
        )
    ...
    safe_query = "".join(
        [c for c in st.session_state.rag_query if c.isalnum() or c in (" ", "-", "_")]
    ).strip()[:30]
...
def _render_input_block(chat_map: dict, all_chats: list) -> None:
    ...
    search_btn = st.button(
        "Analysieren & Antworten 🚀", type="primary", width="stretch"
    )
...
def _run_analysis_pipeline(
    search_query: str, selected_chat_ids, chat_map: dict
) -> None:
    ...
    with st.spinner("1. Suche relevante Fakten..."):
        try:
            ...
            # Imbalance-Warnungen anzeigen (blockieren NICHT!)
            ...
            # Wenn "none" → kein Hinweis nötig, Pipeline läuft weiter!
            ...
            import traceback
            st.code(traceback.format_exc()) # Zeigt den Fehler direkt im Browser an!
