...
def render_pipeline_trace(trace: Optional[Dict[str, Any]] = None) -> None:
    ...
    with st.expander("🔍 Pipeline-Transparenz", expanded=False):
        ...
        st.caption(f"⚖️ Essence Parity: {'✅ Aktiv' if parity else '❌ Inaktiv'}")
        ...
        if chunk_table:
            ...
            df_kept = pd.DataFrame(chunk_table).rename(
                columns={
                    "title": "Dokument",
                    "score": "Score",
                    "rescued": "Gerettet",
            ...
            df_kept["Gerettet"] = df_kept["Gerettet"].apply(lambda x: "🚑" if x else "")
            st.dataframe(
                df_kept,
                width="stretch",
                column_config={
            ...
            if rescued_count:
                st.caption(f"🚑 {rescued_count} Chunk(s) via Rescue Mission gerettet.")
        ...
        with st.expander(
            ...
            df_rejected = pd.DataFrame(rejected_chunks).rename(
                columns={
                    "title": "Dokument",
                    "score": "Score",
                    "date": "Datum",
            ...
            borderline = [c for c in rejected_chunks if c["score"] >= threshold * 0.85]
            if borderline:
                st.warning(
